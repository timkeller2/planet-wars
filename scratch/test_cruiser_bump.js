...
const runTests = () => {
  ...
  ship1.health = 100;
  ship1.speed = 15;
  ship1.package = 'brute'; // Forces shouldCloseIn = true so it attempts to close within 30px
  ...
  for (let tick = 0; tick < 100; tick++) {
    ...
    if (ship1.currentSpeed === 0) {
      ...
      if (dist < 20 || dist > 31.0) {
        console.error(`FAILED: Expected stop distance close to 30px, got ${dist.toFixed(2)}px`);
        process.exit(1);
      }
      stopped = true;
      break;
